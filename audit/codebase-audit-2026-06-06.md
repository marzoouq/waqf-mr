...
| Critical | 0 | 0 | **0** |
| Warning | 0 | 0 | **0** |
| Info | 5 | 4 | **4** (مستقرة — Canvas/PDF) |
| ملفات > 200 LOC | 17 | 17 | **17** (مبرّرة بالكامل) |
| Hooks > 200 LOC في الإنتاج | 4 | 3 | **3** (201-220 — قريبة من الحد) |
| Pages تستورد `hooks/data` | 0 | 0 | **0** |
| `any` خارج الاختبارات | 1 | 1 | **1** (`pdfHelpers.ts`) |

لا تراجع في أي مؤشر. التحسينات السابقة (تقسيم 5 مكونات، استخراج `useAnnualReportExport`، نقل `components/common/*` إلى مجلدات فرعية، إنشاء `lib/pwa/semver`) مستقرة.
