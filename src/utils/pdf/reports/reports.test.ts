...
import { generateAnnualReportPDF, generateBeneficiaryStatementPDF, generateAnnualDisclosurePDF } from '../reports/reports';
...
describe('generateAnnualReportPDF', () => {
  ...
  it('generates and saves PDF', async () => {
    ...
    expect(vi.mocked((await import('../core/core')).finalizePdf).mock.calls[0]?.[2]).toBe('waqf-report-2024-2025.pdf');
  });
});
...
describe('generateBeneficiaryStatementPDF', () => {
  ...
  it('generates and saves PDF', async () => {
    ...
    expect(vi.mocked((await import('../core/core')).finalizePdf)).toHaveBeenCalled();
    expect(vi.mocked((await import('../core/core')).finalizePdf).mock.calls[0]?.[2]).toBe('statement-أحمد-2024-2025.pdf');
  });
});
...
describe('generateAnnualDisclosurePDF', () => {
  ...
  it('generates and saves disclosure PDF', async () => {
    ...
    expect(vi.mocked((await import('../core/core')).finalizePdf)).toHaveBeenCalled();
    expect(vi.mocked((await import('../core/core')).finalizePdf).mock.calls[0]?.[2]).toBe('annual-disclosure-2024-2025.pdf');
  });
});
