...
describe('generateAnnualReportPDF', () => {
  ...
  it('generates and saves PDF', async () => {
    ...
    expect(vi.mocked((await import('./core')).finalizePdf)).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'waqf-report-2024-2025.pdf', expect.anything());
  });
});
...
describe('generateBeneficiaryStatementPDF', () => {
  it('generates and saves PDF', async () => {
    ...
    expect(vi.mocked((await import('./core')).finalizePdf)).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'statement-أحمد-2024-2025.pdf', expect.anything());
  });
});
...
describe('generateAnnualDisclosurePDF', () => {
  it('generates and saves disclosure PDF', async () => {
    ...
    expect(vi.mocked((await import('./core')).finalizePdf)).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'annual-disclosure-2024-2025.pdf', expect.anything());
  });
});
