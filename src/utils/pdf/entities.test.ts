...
describe('entities PDF', () => {
  ...
  it('generatePropertiesPDF saves correctly', async () => {
    ...
    expect(vi.mocked((await import('./core')).finalizePdf)).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'properties-report.pdf', expect.anything());
  });
  ...
  it('generateContractsPDF saves correctly', async () => {
    ...
    expect(vi.mocked((await import('./core')).finalizePdf)).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'contracts-report.pdf', expect.anything());
  });
  ...
  it('generateBeneficiariesPDF saves correctly', async () => {
    ...
    expect(vi.mocked((await import('./core')).finalizePdf)).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'beneficiaries-report.pdf', expect.anything());
  });
