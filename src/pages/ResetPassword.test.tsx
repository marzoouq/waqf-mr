...
const { mockUpdateUser, mockNavigate, mockNotify } = vi.hoisted(() => ({
  mockUpdateUser: vi.fn(),
  mockNavigate: vi.fn(),
  mockNotify: { error: vi.fn(), success: vi.fn(), info: vi.fn(), warning: vi.fn() },
}));
...
describe('ResetPassword', () => {
  ...
  describe('نموذج إعادة التعيين', () => {
    ...
    it('يعرض خطأ عند إرسال كلمة مرور أقل من 8 أحرف', async () => {
      ...
      await waitFor(() => {
        expect(mockNotify.error).toHaveBeenCalledWith('كلمة المرور يجب أن تكون 8 أحرف على الأقل');
      });
    ...
    it('يعرض خطأ عند عدم تطابق كلمتي المرور', async () => {
      ...
      fireEvent.submit(passInput.closest('form')!);
      await waitFor(() => {
        expect(mockNotify.error).toHaveBeenCalledWith('كلمتا المرور غير متطابقتين');
      });
      expect(mockUpdateUser).not.toHaveBeenCalled();
    });
    ...
    it('يعرض رسالة نجاح بعد تغيير كلمة المرور بنجاح', async () => {
      ...
      fireEvent.submit(passInput.closest('form')!);
      await waitFor(() => {
        expect(mockNotify.success).toHaveBeenCalledWith('تم تغيير كلمة المرور بنجاح');
