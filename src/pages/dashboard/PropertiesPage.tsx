        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredProperties.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map((property) => {
              ...
              const rentedUnitIdsForProp = new Set(propContracts.filter(c => c.status === 'active' && c.unit_id).map(c => c.unit_id));
              const isWholePropertyRented = propContracts.some(c => c.status === 'active' && !c.unit_id);
