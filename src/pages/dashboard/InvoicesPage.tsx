          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            {(() => {
              ...
              return withoutFiles.length > 0 ? (
                <Button variant="outline" className="gap-2" disabled={generatePdf.isPending} onClick={() => generatePdf.mutate(withoutFiles.map(inv => inv.id))}>
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => setDeleteTarget({ id: item.id, name: item.file_name || 'فاتورة', file_path: item.file_path })} className="text-destructive hover:text-destructive"><Trash2 className="w-4 h-4" /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
