...
describe('useDistributeShares', () => {
  ...
  it('ينفّذ التوزيع بنجاح عبر RPC', async () => {
    ...
    result.current.mutate({
      account_id: 'acc-1', fiscal_year_id: 'fy-1',
      distributions: [{ beneficiary_id: 'b1', beneficiary_name: 'أحمد', share_amount: 1000, advances_paid: 0, carryforward_deducted: 0, net_amount: 1000, deficit: 0 }],
  ...
  it('يتعامل مع خطأ RPC', async () => {
    ...
    const { result } = renderHook(() => useDistributeShares(), { wrapper: createWrapper() });
    result.current.mutate({ account_id: 'acc-1', distributions: [], total_distributed: 0 });
  ...
  it('يحوّل beneficiary_user_id undefined إلى null', async () => {
    ...
    result.current.mutate({
      account_id: 'acc-1',
      distributions: [{ beneficiary_id: 'b1', beneficiary_name: 'X', beneficiary_user_id: undefined, share_amount: 100, advances_paid: 0, carryforward_deducted: 0, net_amount: 100, deficit: 0 }],
