  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.rental_mode === 'multi') {
      if (formData.selected_unit_ids.length === 0) {
        uiNotify.error('يرجى اختيار وحدة واحدة على الأقل');
        return;
      }
