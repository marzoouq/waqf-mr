...
export function useBeneficiariesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.share_percentage) { uiNotify.error('يرجى ملء جميع الحقول المطلوبة'); return; }
    ...
    if (currentTotal + newPercentage > 100) { uiNotify.error('مجموع نسب المستفيدين يتجاوز 100%'); return; }
  ...
  const handleExportCsv = useCallback(() => {
    ...
    uiNotify.success('تم تصدير المستفيدين بنجاح');
