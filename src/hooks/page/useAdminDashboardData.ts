...
interface UseAdminDashboardDataParams {
  ...
  summary: {
    ...
    allUnits: any[];
    paymentInvoices: any[];
    contractAllocations: any[];
    advanceRequests: any[];
    orphanedContracts: any[];
    income: any[];
    expenses: any[];
    accounts: any[];
    beneficiaries: any[];
...
export const useAdminDashboardData = ({
  ...
  const pendingAdvancesCount = useMemo(
    () => advanceRequests.filter((r: any) => r.status === 'pending').length,
    [advanceRequests],
  );

  // ── الحسابات المالية ──
  const computedAccounts = useMemo(
    () => accounts.map((a: any) => ({ ...a, fiscal_year_id: a.fiscal_year_id ?? '' })),
    [accounts],
  );
  const {
  ...
  const relevantContracts = useMemo(
    () => isSpecificYear ? contracts : contracts.filter((c: any) => c.status === 'active'),
  ...
  const contractualRevenue = useMemo(() => {
    if (isSpecificYear && contractAllocations.length > 0) {
      ...
      contractAllocations.forEach((a: any) => {
        allocMap.set(a.contract_id, (allocMap.get(a.contract_id) ?? 0) + safeNumber(a.allocated_amount));
      });
      return relevantContracts.reduce((sum: number, c: any) => sum + (allocMap.get(c.id) ?? 0), 0);
    }
    return relevantContracts.reduce((sum: number, c: any) => sum + safeNumber(c.rent_amount), 0);
