          ...
          <>
            ...
            <div className="space-y-3 md:hidden">
              {contracts.map((contract, index) => (
                <div key={contract.id} className="p-3 rounded-lg border bg-muted/20 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => onEditContract(contract)} className="p-1.5 rounded hover:bg-muted" aria-label="تعديل"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => onDeleteContract(contract.id, `العقد ${contract.contract_number}`)} className="p-1.5 rounded hover:bg-muted text-destructive" aria-label="حذف"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div><span className="text-muted-foreground">قيمة الدفعة</span><p className="font-bold text-primary">{getPaymentPerPeriod(contract).toLocaleString()} ريال</p></div>
                    <div><span className="text-muted-foreground">الإيجار السنوي</span><p className="font-bold text-primary">{Number(contract.rent_amount).toLocaleString()} ريال</p></div>
                    <div><span className="text-muted-foreground">عدد الدفعات</span><p className="font-medium">{getExpectedPayments(contract)}</p></div>
                    <div><span className="text-muted-foreground">الحالة</span><p className="font-medium">{statusLabel(contract.status)}</p></div>
                  </div>
                </div>
              ))}
              <div className="p-3 rounded-lg bg-muted/70 font-bold text-sm flex justify-between">
                <span>الإجمالي ({contracts.length} عقد)</span>
                <span className="text-primary">{totalAnnualRent.toLocaleString()} ريال</span>
              </div>
            </div>
            {/* Desktop Table */}
