    ...
    <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) { onReset(); setErrors({}); } }}>
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pl-2">
          ...
          <div className="space-y-2"><div className="space-y-2"><Label htmlFor="beneficiary-form-dialog-field-3">ملاحظات</Label><Input name="notes" id="beneficiary-form-dialog-field-3" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="ملاحظات إضافية" maxLength={500} /></div> value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="ملاحظات إضافية" maxLength={500} /></div>
