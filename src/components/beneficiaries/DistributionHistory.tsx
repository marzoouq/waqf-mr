...
import { useDistributionHistory } from '@/hooks/data/useDistributionHistory';
...
const DistributionHistory = ({ beneficiary }: Props) => {
  const { data: distributions = [], isLoading } = useDistributionHistory(beneficiary.id);
  ...
  if (isLoading) {
    return (<Card className="shadow-sm"><CardContent className="p-4 space-y-3"><Skeleton className="h-5 w-40" /><Skeleton className="h-24 w-full" /></CardContent></Card>);
  }

  if (yearlyData.length === 0) {
    return (<Card className="shadow-sm"><CardContent className="p-6 text-center"><History className="w-8 h-8 mx-auto text-muted-foreground mb-2" /><p className="text-sm text-muted-foreground">لا توجد توزيعات سابقة لـ {beneficiary.name}</p></CardContent></Card>);
  }
    ...
    <Card className="shadow-sm">
      ...
      <CardContent className="p-0">
          ...
          {yearlyData.map((row, idx) => {
            ...
            return (
              <div key={row.year} className="flex items-center justify-between gap-3 p-3 rounded-lg border bg-card">
                <div className="min-w-0"><p className="text-sm font-bold">{row.year}</p><p className="text-xs text-muted-foreground">{row.count} دفعات</p></div>
        ...
        <div className="hidden md:block overflow-x-auto">
          <Table>
            <TableHeader><TableRow className="bg-muted/50"><TableHead className="text-right">السنة المالية</TableHead><TableHead className="text-right">المبلغ الإجمالي</TableHead><TableHead className="text-right">عدد الدفعات</TableHead><TableHead className="text-right">التغيّر</TableHead></TableRow></TableHeader>
            <TableBody>
              {yearlyData.map((row, idx) => {
                const prevRow = yearlyData[idx + 1];
                const change = prevRow ? ((row.total - prevRow.total) / prevRow.total) * 100 : null;
