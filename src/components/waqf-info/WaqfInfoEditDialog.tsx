...
import { useWaqfInfoSave } from '@/hooks/page/admin/settings/useWaqfInfoSave';
...
const WaqfInfoEditDialog: React.FC<WaqfInfoEditDialogProps> = ({
  ...
  const handleLogoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ext || !VALID_LOGO_EXT[file.type]?.includes(ext)) {
      uiNotify.error('امتداد الملف لا يتطابق مع نوعه');
      return;
    }
    ...
    try {
      ...
      if (result.wasResized) {
        uiNotify.success(`تم تصغير الشعار تلقائياً (${result.originalWidth}×${result.originalHeight} → ${result.newWidth}×${result.newHeight})`);
      }
    } catch {
      uiNotify.error('فشل معالجة الصورة');
    } finally {
      setResizing(false);
    }
  };
