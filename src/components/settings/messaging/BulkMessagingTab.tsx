...
import { useBeneficiariesForMessaging } from '@/hooks/data/messaging/useBulkMessaging';
import { useBulkMessageSender } from '@/hooks/page/admin/messaging/useBulkMessageSender';
...
const BulkMessagingTab = () => {
  ...
  const handleSend = async () => {
    if (!message.trim()) { uiNotify.error('يرجى كتابة نص الرسالة'); return; }

    const recipients = target === 'all'
      ? beneficiaries
      : beneficiaries.filter(b => selectedIds.includes(b.id));

    if (recipients.length === 0) { uiNotify.error('يرجى اختيار مستفيد واحد على الأقل'); return; }
