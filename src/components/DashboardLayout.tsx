...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex w-full bg-background">
      ...
      <aside
        className={cn(
          'fixed inset-y-0 right-0 z-50 flex flex-col gradient-hero transition-all duration-300 shadow-elegant',
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-sidebar-border">
          <div className={cn('flex items-center gap-3', !sidebarOpen && 'justify-center')}>
            ...
            {sidebarOpen && <span className="font-bold text-sidebar-foreground">إدارة الوقف</span>}
          </div>
          <Button
            variant="ghost"
            size="icon"
      ...
      {/* Main Content */}
      <main className={cn('flex-1 transition-all duration-300', sidebarOpen ? 'mr-64' : 'mr-16')}>
        {children}
      </main>
    </div>
  );
