/**
 * بطاقة عقار — مستخرجة من PropertiesPage لتقليل حجم الملف
 */
...
const PropertyCard = ({ property, financials, contracts, isSpecificYear, onEdit, onDelete, onClick }: PropertyCardProps) => {
  const {
    totalUnits, rented, vacant, maintenance, statusMismatch,
    occupancy, occupancyColor, progressColor,
    monthlyRent, activeAnnualRent, totalExpenses, netIncome, contractualRevenue,
  } = financials;

  return (
    <Card className="shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => onClick(property)}>
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-1">
            ...
            <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); onDelete(property); }} className="text-destructive hover:text-destructive" aria-label="حذف العقار"><Trash2 className="w-4 h-4" /></Button>
          </div>
        </div>
      </CardHeader>
