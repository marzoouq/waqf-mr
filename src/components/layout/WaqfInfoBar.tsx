...
const WaqfInfoBar = () => {
  ...
  const prevLogoUrlRef = useRef(waqfInfo?.waqf_logo_url);
  if (prevLogoUrlRef.current !== waqfInfo?.waqf_logo_url) {
    prevLogoUrlRef.current = waqfInfo?.waqf_logo_url;
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer group">
                {waqfInfo.waqf_logo_url && !logoError ? (
                  <img
                    src={waqfInfo.waqf_logo_url}
                    alt="شعار الوقف"
