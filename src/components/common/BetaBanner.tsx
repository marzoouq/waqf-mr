...
const BetaBanner = () => {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`${isBottom ? "fixed bottom-0 left-0 right-0" : ""} z-60 flex items-center justify-center gap-2 ${isHidden ? '' : colors.bg} px-4 py-1.5 text-primary-foreground text-sm font-medium shadow-md print:hidden className={`${isBottom ? "fixed bottom-0 left-0 right-0" : ""} z-60 flex items-center justify-center gap-2 ${isHidden ? '' : colors.bg} px-4 py-1.5 text-primary-foreground text-sm font-medium shadow-md print:hidden transition-[height,padding] duration-200`}`}
