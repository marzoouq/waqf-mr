...
import { Home, Building2, FileText, Wallet, Menu, ClipboardList, Receipt } from 'lucide-react';
...
const BottomNav: React.FC<BottomNavProps> = ({ onOpenSidebar }) => {
  ...
  const navLinks = role === 'admin' || role === 'accountant'
    ? adminLinks
    : role === 'waqif'
      ? waqifLinks
      : beneficiaryLinks;
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden border-t border-border/50 bg-background/80 backdrop-blur-lg safe-area-bottom">
