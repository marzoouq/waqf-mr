    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <Button variant="outline" size="sm" className="gap-2" onClick={handleSendAlerts} disabled={sendingAlerts}>
          <Bell className="w-4 h-4" />{sendingAlerts ? 'جاري الإرسال...' : 'إرسال تنبيهات التأخير'}
        </Button>
      ...
      <Card className="shadow-sm">
        <CardContent className="p-0">
          {filteredRows.length === 0 ? (
            ...
            <>
              <CollectionMobileCards rows={paginatedRows} />
              <CollectionDesktopTable rows={paginatedRows} expectedLabel={expectedLabel} />
