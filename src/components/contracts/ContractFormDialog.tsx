    ...
    <Dialog open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) { setFormData(emptyFormData); onReset(); } }}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-3 rounded-lg bg-muted/50 text-sm space-y-1">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-primary">
                  {fmt(parseFloat(formData.rent_amount) / (formData.payment_type === 'monthly' ? 12 : formData.payment_type === 'annual' ? 1 : (parseInt(formData.payment_count) || 1)))} ر.س
                </span>
              </div>
                ...
                <>
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-muted-foreground">
                      {fmt((parseFloat(formData.rent_amount) / (formData.payment_type === 'monthly' ? 12 : formData.payment_type === 'annual' ? 1 : (parseInt(formData.payment_count) || 1))) * 0.15)} ر.س
                    </span>
                  </div>
          ...
          <div className="space-y-2"><div className="space-y-2"><Label htmlFor="contract-form-dialog-field-5">ملاحظات</Label><Input name="notes" id="contract-form-dialog-field-5" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="ملاحظات إضافية" /></div> value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="ملاحظات إضافية" /></div>
