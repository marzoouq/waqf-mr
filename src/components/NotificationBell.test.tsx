...
describe('NotificationBell', () => {
  ...
  it('shows unread count badge', () => {
    mockedUseNotifications.mockReturnValue({ data: [], filteredData: [], unreadCount: 5, filteredUnreadCount: 5, ...baseMock } as ReturnType<typeof useNotifications>);
    renderBell();
    expect(screen.getByText('5')).toBeInTheDocument();
  });

  it('shows 9+ when unread count exceeds 9', () => {
    mockedUseNotifications.mockReturnValue({ data: [], filteredData: [], unreadCount: 15, filteredUnreadCount: 15, ...baseMock } as ReturnType<typeof useNotifications>);
    renderBell();
    expect(screen.getByText('9+')).toBeInTheDocument();
  });

  it('shows empty message when popover opened with no notifications', async () => {
    mockedUseNotifications.mockReturnValue({ data: [], filteredData: [], unreadCount: 0, filteredUnreadCount: 0, ...baseMock } as ReturnType<typeof useNotifications>);
    renderBell();
