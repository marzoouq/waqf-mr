# تقرير التدقيق المعماري — فحص جنائي شامل (read-only)

## TL;DR — الحالة العامة: **ممتازة (A-)**

بعد فحص 1,137 ملف TypeScript (~92K سطر)، الكود يحترم القواعد بمستوى نادر:


| فحص                                       | النتيجة                     |
| ----------------------------------------- | --------------------------- |
| `supabase` خام في `pages/components`      | **0** ✅                     |
| `supabase` أو `sonner` في `utils/`        | **0** ✅ (README فقط)        |
| `console.*` خارج logger                   | **0** ✅                     |
| `#hex` خارج Canvas                        | **0** ✅                     |
| `localStorage` لـ fiscal_year             | **0** ✅                     |
| `getSession()` في edge functions          | **0** ✅                     |
| `: any` خارج types/tests                  | **2** ✅                     |
| أكبر ملف غير مُولَّد/اختبار               | **216 سطر** ✅ (تحت سقف 250) |
| Route-level ErrorBoundary + lazyWithRetry | **مُطبَّق** ✅               |
| Page Hook Pattern (pages بدون Supabase)   | **مُطبَّق 100%** ✅          |


**لا توجد إصلاحات حرجة (P0). كل البنود التالية تحسينات.**

---

## 1. الثغرات المنهجية الحقيقية (P1 — توصية بالإصلاح)

### 1.1 تغطية اختبارات ضعيفة في طبقات حرجة


| طبقة                | ملفات | اختبارات | تغطية     |
| ------------------- | ----- | -------- | --------- |
| `hooks/page`        | 81    | 6        | **7%** ⚠️ |
| `hooks/application` | 13    | 0        | **0%** ⚠️ |
| `hooks/auth`        | 21    | 2        | **9%** ⚠️ |
| `lib/`              | 64    | 11       | 17%       |
| `components/`       | 377   | 43       | 11%       |
| `hooks/data`        | 98    | 32       | 32%       |
| `utils/`            | 91    | 49       | **53%** ✅ |


**أثر:** Page Hooks تنسّق التدفقات الحساسة (Distribution, Closure, ZATCA Onboard) — أي ريغرشن صامت يمر بدون شبكة أمان. الموجة 4 الحالية ضخّمت كود `hooks/page/` بدون اختبارات مرافقة.

**التوصية:** موجة اختبارات مستهدفة:

- 8 page hooks مالية حرجة: `useAccountsPage`, `useDistributionPage`, `useFiscalYearClosurePage`, `useZatcaSettings`, `useAdvanceRequestsPage`, `useInvoicePaymentsPage`, `useAnnualReportPage`, `useExpensesPage`.
- 3 application controllers: `useDashboardSummary`, `useMessagingThread`, `useSupportRouting`.
- 2 auth flows: `useLoginFlow`, `useBiometricLoginFlow`.

### 1.2 ضعف عقود الأنواع في المكوّنات

- **290 من 377** مكوّن بلا `interface XxxProps` صريح. كثير من الـ leaf components تأخذ inline props (`{ a, b }: { a: T; b: U }`) — مقبول، لكن في الـ container components يجعل refactor مكلفاً.
- **التوصية:** ESLint rule على `src/components/**/*.tsx` تتطلب `interface|type` مفصول للمكوّنات التي تأخذ ≥ 3 خصائص، أو ≥ 80 سطر.

### 1.3 طبقة `lib/` فوضوية على المستوى الجذري

`src/lib/` يخلط بين:

- **مجلدات نظيفة:** `api/`, `auth/`, `hooks/`, `services/`, `realtime/`, `notifications/`, `monitoring/`, `navigation/`, `search/`, `storage`, `theme/`, `queryKeys/`, `diagnostics/`.
- **11 ملف سائب على الجذر:** `cn.ts`, `logger.ts`, `notify.ts`, `storage.ts`, `lazyWithRetry.ts`, `errorReporter.ts`, `initQueryMonitoring.ts`, `pwaBootstrap.ts`, `queryClient.ts`, `queryStaleTime.ts`, `componentPrefetch.ts`.

**التوصية:** إعادة تنظيم بدون تغيير عام للـ imports — استبقاء re-exports من الجذر للتوافق:

```
lib/
  bootstrap/  → pwaBootstrap, initQueryMonitoring, componentPrefetch
  query/      → queryClient, queryStaleTime
  ui/         → cn, notify
  error/      → errorReporter, logger
  router/     → lazyWithRetry
  fs/         → storage
```

---

## 2. تحسينات بنيوية (P2 — مرغوب)

### 2.1 توحيد `components/shared/` و `components/common/`

حالياً:

- `components/common/` — 27 ملف (مكوّنات عامة: ErrorBoundary, DeferredRender, EmptyState, ChartBox...).
- `components/shared/dashboard/DashboardLazySection.tsx` — ملف واحد فقط.

**التوصية:** نقل `DashboardLazySection.tsx` إلى `components/common/` وحذف مجلد `shared/` لإلغاء الازدواجية المفاهيمية.

### 2.2 `DeferredRender` ناقص الاستخدام

- يُستخدم في **7 ملفات فقط** بينما الـ memory rule تطلب استخدامه للمكوّنات الثانوية.
- المرشحون البديهيون: charts خارج viewport, modals ثقيلة, panels تحت الـ scroll.

**التوصية:** audit بصري لخمس صفحات (`AdminDashboard`, `AccountsPage`, `ReportsPage`, `BeneficiariesPage`, `InvoicesPage`) لتحديد المكوّنات المؤجلة.

### 2.3 `eslint-disable` بدون تعليل

- **41 موضع** عبر `src/`. القاعدة الذهبية: كل `eslint-disable` يجب أن يلي تعليق بسبب صريح.
- **التوصية:** ESLint custom rule تطلب JSDoc-style reason أمام كل disable.

### 2.4 صفحات بدون hook خاص

البحث الحالي يثبت أن كل الصفحات تستخدم page hooks، لكن لم نتحقق من **حجم الصفحات نفسها**:

- **التوصية:** فحص `find src/pages -name '*.tsx' | xargs wc -l` وتحديد الصفحات > 150 سطر لإعادة تنظيمها (لم يتم في هذا التدقيق لاختصار المسح).

---

## 3. تحسينات تشغيلية (P3 — اختياري)

### 3.1 أرشفة الـ migrations

- **331 migration** في `supabase/migrations/`. عمليات `bun install` و IDE indexing تبطؤ.
- **التوصية:** نسخة snapshot موحّدة لكل ما قبل تاريخ X في `supabase/migrations/_archive/` (read-only). الاحتفاظ بآخر 60 migration نشطة فقط.

### 3.2 `index.css` 593 سطر

- 3 طبقات فقط (`@layer`) — حجم معقول، لكن يحتوي على tokens + utilities + keyframes مختلطة.
- **التوصية اختيارية:** تقسيم إلى `tokens.css`, `utilities.css`, `animations.css` تستوردها `index.css`.

### 3.3 محتوى مكرر محتمل

- **29 مكوّن** ينتهي بـ `Card$`. لم يثبت تكرار سلوكي، لكن مرشح لـ pattern موحّد (`<KpiCard>`, `<MetricCard>`).
- لم نجد ملفّين بنفس الاسم — أي لا توجد ازدواجية ملفات مباشرة.

---

## 4. ما ليس مكسوراً (لا تَلمس)

- ✅ بنية routing (`adminRoutes`/`beneficiaryRoutes`/`waqifRoutes` + `pr()` helper + RouteGuard).
- ✅ Edge Functions: 19 وظيفة، `_shared/auth.ts` يفرض `getUser()`.
- ✅ RLS: كل جدول مالي محمي بـ `is_fiscal_year_accessible` كسياسة RESTRICTIVE.
- ✅ Page Hook Pattern مطبَّق على كل الصفحات (لا supabase خام).
- ✅ Lazy loading لكل الـ pages.
- ✅ `React.memo` على table rows فقط (مطابق للقاعدة).

---

## 5. خطة تنفيذ موجَّهة بالأولوية


| #   | البند                                                 | الموجة المقترحة | تقدير                      |
| --- | ----------------------------------------------------- | --------------- | -------------------------- |
| 1   | اختبارات page hooks مالية (8 hooks)                   | Wave T-1        | جلسة منفصلة                |
| 2   | اختبارات application + auth (5 hooks)                 | Wave T-2        | جلسة منفصلة                |
| 3   | إعادة تنظيم `src/lib/` مع re-exports للتوافق          | Wave R-1        | جلسة منفصلة                |
| 4   | توحيد `shared/` ضمن `common/`                         | Wave R-2        | جلسة قصيرة                 |
| 5   | ESLint rule: Props interface إلزامي للمكوّنات الكبيرة | Wave Q-1        | جلسة قصيرة                 |
| 6   | ESLint rule: تعليل إجباري لكل `eslint-disable`        | Wave Q-1        | مع #5                      |
| 7   | audit صفحات > 150 سطر + DeferredRender expansion      | Wave R-3        | جلسة منفصلة                |
| 8   | أرشفة migrations القديمة                              | Wave Ops-1      | جلسة منفصلة + تأكيد الناظر |
| 9   | تقسيم `index.css` (اختياري)                           | Wave Ops-2      | جلسة قصيرة                 |


---

## 6. ما لم يتم فحصه في هذه الجلسة (للشفافية)

- لم نُشغّل `bun audit` ولا `supabase--linter` (read-only mode + لتجنّب تجاوز السقف).
- لم نتحقق من حجم كل صفحة `src/pages/**` بنداً بنداً.
- لم نُجرِ Lighthouse/perf profile على المعاينة.
- لم نتفقّد دلالات كل RLS policy واحدة واحدة (اعتمدنا على بنيتها).

ضمن أي موجة قادمة سأُضيف هذه الفحوصات تلقائياً قبل التنفيذ.

---

**لا أعدِّل أي شيء الآن** — هذه خطة تدقيق فقط. عند الموافقة، أبدأ بـ **Wave T-1 (اختبارات page hooks مالية)** كأكثر بند يخفض المخاطر التشغيلية.  
التوصيات المطلوبه اثناء التنفيذ   
**Error Fixing**
