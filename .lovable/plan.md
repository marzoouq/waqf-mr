# خطة T14 — دمج Migrations في Baseline واحد

> **حالة:** ⛔ **غير موصى بها حالياً** — راجع قسم "المراجعة" أدناه.
> **آخر تحديث:** 2026-05-28

## مراجعة 2026-05-28 (قرار: تأجيل غير محدد)

بعد فحص فعلي للحالة:
- **حجم migrations في المستودع:** 858 كيلوبايت فقط (339 ملف) — غير مؤثر على git أو CI
- **ملفات فارغة/تالفة:** 0
- **migrations مُطبّقة في Live:** 445 (تتضمن migrations تاريخية قبل الـ 339 الحالية)
- **الفجوة بين Live والمستودع:** 106 migration قديمة مُطبّقة لكن غير موجودة في الريبو

**النتيجة:** المخاطرة العالية لـ T14 (احتمال كسر تتبع Supabase، فقدان بيانات، عدم تطابق Test/Live) **لا تبررها** فائدة عملية محسوسة. حجم 858KB لا يُبطئ أي شيء.

**التوصية:** الإبقاء على migrations كما هي. إعادة النظر فقط عند بلوغ أحد العتبات التالية:
- 1000+ ملف migration
- حجم تراكمي > 10 MB
- بطء ملموس في `supabase db reset` المحلي (> دقيقة)
- الحاجة لإعداد بيئة dev جديدة بشكل متكرر

---

## (المحتوى الأصلي للخطة محفوظ أدناه كمرجع)

## السياق
- عدد ملفات migrations الحالية: **339 ملف** في `supabase/migrations/`
