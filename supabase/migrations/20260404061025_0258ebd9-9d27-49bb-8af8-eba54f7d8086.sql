...
BEGIN
  ...
  LOOP
    -- جلب معلومات السنة
    SELECT id, label, status INTO v_fy
    FROM fiscal_years WHERE id = v_year_id;

    IF v_fy.id IS NULL THEN CONTINUE; END IF;

    -- إجمالي الدخل والمصروفات
    SELECT COALESCE(SUM(amount), 0) INTO v_total_income
    FROM income WHERE fiscal_year_id = v_year_id;

    SELECT COALESCE(SUM(amount), 0) INTO v_total_expenses
    FROM expenses WHERE fiscal_year_id = v_year_id;

    -- بيانات الحساب المقفل
...
-- 2) تعديل get_beneficiary_dashboard لإضافة monthly_expenses
...
BEGIN
  ...
  IF v_fy.id IS NOT NULL THEN
    ...
    -- جديد: تجميع المصروفات حسب الشهر
    SELECT COALESCE(jsonb_agg(jsonb_build_object('month', sub.m, 'total', sub.t) ORDER BY sub.m), '[]'::jsonb)
