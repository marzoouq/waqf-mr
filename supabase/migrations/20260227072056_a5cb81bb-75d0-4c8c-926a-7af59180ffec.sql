CREATE OR REPLACE FUNCTION public.execute_distribution(p_account_id uuid, p_fiscal_year_id uuid DEFAULT NULL::uuid, p_total_distributed numeric DEFAULT 0, p_distributions jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ✅ FIXED: Idempotency guard now always uses fiscal_year_id (never NULL)
  IF EXISTS (
