...
BEGIN
  ...
  -- 2) إجمالي نسب المستفيدين + عدد المستفيدين (#12)
  SELECT COALESCE(SUM(share_percentage), 0), COUNT(*)
    INTO v_total_pct, v_ben_count
    FROM beneficiaries;

  -- 2b) نسب الحصص من app_settings (#13)
  ...
  -- حساب إجمالي السُلف المدفوعة للسنة الحالية
