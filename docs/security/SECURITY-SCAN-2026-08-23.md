...
### ملاحظات تخزين الفواتير — تصحيح 2026-08-25

> **تصحيح مهم:** الصياغة الأصلية لهذا القسم كانت خاطئة. اعتُبرت النتيجتان أدناه
> "false positives" لأن التحقق جرى على قاعدة **بيئة التطوير** فقط، بينما كانت
> السياستان **حيّتين فعلياً في الإنتاج** لأن هجرات 2026-08-02 لم تكن قد طُبِّقت
> على الإنتاج بعد. التفاصيل في `docs/security/incident-2026-08-25-migration-drift.md`.
...
الوضع الصحيح: كلتا السياستين مُسقَطتان في بيئة التطوير، ومُسقَطتان في الإنتاج
بعد نشر هجرات 2026-08. هجرة 2026-08-25 تُعيد إسقاطهما بشكل idempotent وتضيف
حارس `DO $$` يفشل الترحيل إن عادت أي سياسة قراءة على الحاوية.

المسار الوحيد للتنزيل: Edge Function `invoice-file-url` تتحقق خادمياً من الجلسة والدور والسنة المالية المنشورة وحجب IP وحد المعدل (30/دقيقة)، ثم تُصدر رابطاً موقّعاً 120 ثانية وتُسجّل كل محاولة في `access_log` (`invoice_download` / `invoice_download_denied`).
